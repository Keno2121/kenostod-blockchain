...
// UTL FeeCollector v1.1 — Fixes UTL-002 (receive() miscounting) and UTL-008 (fee transparency)
// Changes from v1.0:
//   - receive() no longer inflates totalFeesCollectedNative or totalTransactionsProcessed
//   - Added EthDeposited event for direct ETH deposits (separate from fee events)
//   - Added calculateFeeBreakdown() for UI transparency
//   - Added totalDirectDeposits tracking for accounting clarity
...
contract UTLFeeCollector is Ownable, ReentrancyGuard {
    ...
    // FIX UTL-002: Track direct deposits separately from fee revenue
    ...
    event FeesForwarded(address indexed destination, uint256 nativeAmount, uint256 timestamp);
    event TokenFeesForwarded(address indexed destination, address indexed token, uint256 amount, uint256 timestamp);
    event FeeRateUpdated(uint256 oldRate, uint256 newRate);
    event TreasuryUpdated(address oldTreasury, address newTreasury);
    event TokenAdded(address indexed token);
    event TokenRemoved(address indexed token);
    // FIX UTL-002: Separate event for non-fee ETH deposits
    ...
    // FIX UTL-008: Transparent fee breakdown for UI display
    function calculateFeeBreakdown(uint256 amount) external view returns (
        uint256 fee,
        uint256 effectiveRateBps,
        bool minFeeApplied,
        bool maxFeeApplied
    ) {
        uint256 rawFee = (amount * feeRate) / FEE_DENOMINATOR;
        fee = rawFee;
        minFeeApplied = false;
        maxFeeApplied = false;

        if (fee < minFee) { fee = minFee; minFeeApplied = true; }
        if (fee > maxFee) { fee = maxFee; maxFeeApplied = true; }
        if (fee > amount) fee = amount;

        effectiveRateBps = amount > 0 ? (fee * FEE_DENOMINATOR) / amount : 0;
        return (fee, effectiveRateBps, minFeeApplied, maxFeeApplied);
    }
    ...
    // FIX UTL-002: receive() no longer inflates fee counters
    // Direct ETH deposits are tracked separately and emit a distinct event
