...
// UTL Treasury v1.1 — Fixes UTL-004 (missing event in distributeTokenFunds)
// Changes from v1.0:
//   - Added TokenFundsDistributed event
//   - distributeTokenFunds() now emits TokenFundsDistributed with full accounting
//   - Added totalTokenDistributed mapping per token for historical tracking
...
contract UTLTreasury is Ownable, ReentrancyGuard {
    ...
    uint256 public kenostodAllocation = 375;
    uint256 public scholarshipAllocation = 250;
    uint256 public tdirAllocation = 250;
    uint256 public insuranceAllocation = 125;

    uint256 public totalReceived;
    uint256 public totalDistributed;
    // FIX UTL-004: Per-token distribution tracking
    ...
    // FIX UTL-004: New event for token distributions
    event TokenFundsDistributed(
    ...
    function distributeNativeFunds() external onlyAuthorized nonReentrant {
        ...
        emit FundsDistributed(kenostodAmount, scholarshipAmount, tdirAmount, insuranceAmount, block.timestamp);
    }

    // FIX UTL-004: Now emits TokenFundsDistributed event with full breakdown
    function distributeTokenFunds(address token) external onlyAuthorized nonReentrant {
        ...
        totalTokenDistributed[token] += balance;

        emit TokenFundsDistributed(token, kenostodAmount, scholarshipAmount, tdirAmount, insuranceAmount, block.timestamp);
    }
