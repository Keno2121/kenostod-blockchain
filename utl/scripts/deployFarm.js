...
async function main() {
    ...
    console.log(
        `  Reward Rate:  ${hre.ethers.formatEther(INITIAL_REWARD_RATE)} KENO/sec (864 KENO/day)`,
    );
    console.log("=".repeat(62));

    if (chainId !== 56 && chainId !== 97) {
        throw new Error(
            `Unexpected chain ID ${chainId}. Expected 56 (BSC) or 97 (BSC Testnet).`,
        );
    }
    if (balance < hre.ethers.parseEther("0.02")) {
        throw new Error(
            `Deployer balance too low (${hre.ethers.formatEther(balance)} BNB). Need at least 0.02 BNB for gas.`,
        );
    }
    ...
    const farm = await UTLFarm.deploy(
        KENO_TOKEN,
        LP_TOKEN,
        INITIAL_REWARD_RATE,
    );
    ...
    console.log(
        `       "${KENO_TOKEN}" "${LP_TOKEN}" "${INITIAL_REWARD_RATE}"`,
    );
